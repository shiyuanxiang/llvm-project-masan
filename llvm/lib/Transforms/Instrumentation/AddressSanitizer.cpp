...
// [syx] interesting memory operands: load, store, atomicrmw, cmpxchg, masked
...
void AddressSanitizer::instrumentMaskedLoadOrStore(
  ...
  SplitBlockAndInsertForEachLane(
      EVL, LoopInsertBefore, [&](IRBuilderBase &IRB, Value *Index) {
        ...
        if (isa<VectorType>(Addr->getType())) {
          ...
          InstrumentedAddress =
              IRB.CreateGEP(Type::getInt8Ty(*C), Addr, {Index});
        } else {
...
void AddressSanitizer::instrumentMop(ObjectSizeOffsetVisitor &ObjSizeVis,
                                     InterestingMemoryOperand &O, bool UseCalls,
                                     const DataLayout &DL) {
  //[syx] get the address of memory operation
  Value *Addr = O.getPtr();
...
void AddressSanitizer::instrumentAddress(Instruction *OrigIns,
  ...
  if (UseCalls && ClOptimizeCallbacks) {
    ...
    //[syx] Intrinsic::asan_check_memacces
    IRB.CreateCall(
...
bool ModuleAddressSanitizer::InstrumentGlobals(IRBuilder<> &IRB, Module &M,
  ...
  *CtorComdat = false;
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    //[syx] NewGlobal is the new global variable with redzone
...
bool AddressSanitizer::instrumentFunction(Function &F,
  ...
  // [syx] runOnFunction, deal with alloca like.
